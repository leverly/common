...
func (this *SkipList) Set(key, value interface{}) {
	next := this.findGE(key, this.path)
...
func (this *SkipList) Delete(key interface{}) (interface{}, bool) {
	node := this.findGE(key, this.path)
	if node != nil && this.compare(key, node.key) == 0 {
		this.length--
...
func (this *SkipList) findGE(key interface{}, path []*SkipListItem) *SkipListItem {
	level := this.height - 1
	cur := this.header
